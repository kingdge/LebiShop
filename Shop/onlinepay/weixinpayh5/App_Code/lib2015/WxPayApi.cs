    ...
    public class WxPayApi
    {
        ...
        */
        public static WxPayData OrderQuery(WxPayData inputObj, int timeOut = 6)
        {
            ...
            ////SystemLog.Add("WxPayApi"+ "OrderQuery request : " + xml);
            string response = HttpService.Post(xml, url, false, timeOut);//调用HTTP通信接口提交数据
            ////SystemLog.Add("WxPayApi"+ "OrderQuery response : " + response);
        ...
        */
        public static WxPayData Refund(WxPayData inputObj, int timeOut = 6)
        {
            ...
            ////SystemLog.Add("WxPayApi"+ "Refund request : " + xml);
            string response = HttpService.Post(xml, url, true, timeOut);//调用HTTP通信接口提交数据到API
            ////SystemLog.Add("WxPayApi"+ "Refund response : " + response);
	    ...
	    */
	    public static WxPayData ShortUrl(WxPayData inputObj, int timeOut = 6)
	    {
            ...
            //SystemLog.Add("WxPayApi"+ "ShortUrl request : " + xml);
            string response = HttpService.Post(xml, url, false, timeOut);
            //SystemLog.Add("WxPayApi"+ "ShortUrl response : " + response);
        ...
        */
        public static WxPayData UnifiedOrder(WxPayData inputObj, int timeOut = 20)
        {
            ...
            inputObj.SetValue("spbill_create_ip", WxPayConfig.IP);//终端ip	  	    
            inputObj.SetValue("nonce_str", GenerateNonceStr());//随机字符串
            //SystemLog.Add("weixinpay-WxPayConfig" + "MCHID : " + conf.MCHID + "，KEY : " + conf.NOTIFY_URL + "，APPID : " + conf.APPID);
            //签名
        ...
        * 根据当前系统时间加随机序列来生成订单号
         * @return 订单号
        */
        public static string GenerateOutTradeNo(Lebi_Order order)
        {
            //string Order_Code = inputObj.GetValue("out_trade_no").ToString();
            //Lebi_Order order = B_Lebi_Order.GetModel("Code = lbsql{'" + Order_Code + "'}");
            //if (order == null)
            ...
            WxPayConfig conf = new WxPayConfig(order);
            var ran = new Random();
            return string.Format("{0}{1}{2}", conf.MCHID, DateTime.Now.ToString("yyyyMMddHHmmss"), ran.Next(999));
        }
