...
{
	/// <summary>
	/// 数据访问类D_Lebi_Supplier_Config。
	/// </summary>
	public partial class D_Lebi_Supplier_Config
	{
		static D_Lebi_Supplier_Config _Instance;
		public static D_Lebi_Supplier_Config Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            _Instance = new D_Lebi_Supplier_Config("Lebi_Supplier_Config");
		        }
		        return _Instance;
		    }
		    set
		...
		string TableName = "Lebi_Supplier_Config";
		public D_Lebi_Supplier_Config(string tablename)
		{
		    TableName = tablename;
		}
		...
		public string GetValue(string colName,string strWhere,int seconds=0)
		{
		   ...
		   try
		   {
		       ...
		       if (cachekey != "")
		           LB.DataAccess.DB.Instance.SetMemchche(cachekey, val, "Lebi_Supplier_Config", 0 , cachestr , seconds);
		   }
		   catch
		   {
		       val = "";
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Lebi_Supplier_Config model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into "+ TableName + " (");
			strSql.Append(LB.DataAccess.DB.BaseUtilsInstance.ColName("Name")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Supplier_id")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Value")+")");
			strSql.Append(" values (");
			strSql.Append("@Name,@Supplier_id,@Value);select @@identity;");
		...
		/// 更新一条数据
		/// </summary>
		public void Update(Lebi_Supplier_Config model)
		{
			StringBuilder strSql=new StringBuilder();
			...
			if((","+model.UpdateCols+",").IndexOf(",Value,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("Value")+"= @Value");
		...
		/// 得到一个对象实体 by id
		/// </summary>
		public Lebi_Supplier_Config GetModel(int id, int seconds=0)
		{
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as Lebi_Supplier_Config;
		   }
		   Lebi_Supplier_Config model = null;
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow,  strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           ...
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Supplier_Config",id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by where条件
		/// </summary>
		public Lebi_Supplier_Config GetModel(string strWhere, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, "", "");
		   ...
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow, strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           model = ReaderBind(dataReader);
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Supplier_Config",model.id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_Supplier_Config GetModel(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldShow = "*";
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_Supplier_Config> GetList(string strWhere, string strFieldOrder, int PageSize, int page, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as List<Lebi_Supplier_Config>;
		   }
		   List<Lebi_Supplier_Config> list = new List<Lebi_Supplier_Config>();
		   ...
		   if (cachekey != "" && list.Count > 0)
		         LB.DataAccess.DB.Instance.SetMemchche(cachekey, list, "Lebi_Supplier_Config", 0 , cachestr , seconds);
		   return list;
		}
		public List<Lebi_Supplier_Config> GetList(SQLPara para, int PageSize, int page, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldKey = "id";
		   string strFieldShow = "*";
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_Supplier_Config> GetList(string strWhere,string strFieldOrder, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
		...
		public List<Lebi_Supplier_Config> GetList(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   StringBuilder strSql=new StringBuilder();
		...
		/// 绑定对象表单
		/// </summary>
		public Lebi_Supplier_Config BindForm(Lebi_Supplier_Config model)
		{
			...
			if (HttpContext.Current.Request["Value"] != null)
				model.Value=LB.Tools.RequestTool.RequestString("Value");
				return model;
		}
		/// <summary>
		/// 安全方式绑定对象表单
		/// </summary>
		public Lebi_Supplier_Config SafeBindForm(Lebi_Supplier_Config model)
		{
			if (HttpContext.Current.Request["Name"] != null)
			...
			if (HttpContext.Current.Request["Supplier_id"] != null)
				model.Supplier_id=LB.Tools.RequestTool.RequestInt("Supplier_id",0);
			if (HttpContext.Current.Request["Value"] != null)
				model.Value=LB.Tools.RequestTool.RequestSafeString("Value");
				return model;
		}
		...
		/// 对象实体绑定数据
		/// </summary>
		public Lebi_Supplier_Config ReaderBind(IDataReader dataReader)
		{
			Lebi_Supplier_Config model=new Lebi_Supplier_Config();
			object ojb; 
			ojb = dataReader["id"];
