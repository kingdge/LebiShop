...
namespace DB.LebiShop
{
	...
	/// </summary>
	public partial class D_Lebi_ServicePanel
	{
		static D_Lebi_ServicePanel _Instance;
		public static D_Lebi_ServicePanel Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            _Instance = new D_Lebi_ServicePanel("Lebi_ServicePanel");
		        }
		        return _Instance;
		    }
		    set
		...
		string TableName = "Lebi_ServicePanel";
		public D_Lebi_ServicePanel(string tablename)
		{
		    TableName = tablename;
		}
		...
		public string GetValue(string colName,string strWhere,int seconds=0)
		{
		   ...
		   try
		   {
		       ...
		       if (cachekey != "")
		           LB.DataAccess.DB.Instance.SetMemchche(cachekey, val, "Lebi_ServicePanel", 0 , cachestr , seconds);
		   }
		   catch
		   {
		       val = "";
		...
		public int Add(Lebi_ServicePanel model)
		{
			...
			strSql.Append(LB.DataAccess.DB.BaseUtilsInstance.ColName("Account")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Face")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Language")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Language_ids")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Name")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("ServicePanel_Group_id")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("ServicePanel_Type_id")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Sort")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Supplier_id")+")");
			strSql.Append(" values (");
			strSql.Append("@Account,@Face,@Language,@Language_ids,@Name,@ServicePanel_Group_id,@ServicePanel_Type_id,@Sort,@Supplier_id);select @@identity;");
		...
		public void Update(Lebi_ServicePanel model)
		{
			...
			if((","+model.UpdateCols+",").IndexOf(",Account,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("Account")+"= @Account");
			...
			if((","+model.UpdateCols+",").IndexOf(",ServicePanel_Group_id,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("ServicePanel_Group_id")+"= @ServicePanel_Group_id");
			if((","+model.UpdateCols+",").IndexOf(",ServicePanel_Type_id,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("ServicePanel_Type_id")+"= @ServicePanel_Type_id");
		...
		/// 得到一个对象实体 by id
		/// </summary>
		public Lebi_ServicePanel GetModel(int id, int seconds=0)
		{
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as Lebi_ServicePanel;
		   }
		   Lebi_ServicePanel model = null;
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow,  strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           ...
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_ServicePanel",id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by where条件
		/// </summary>
		public Lebi_ServicePanel GetModel(string strWhere, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, "", "");
		   ...
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow, strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           model = ReaderBind(dataReader);
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_ServicePanel",model.id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_ServicePanel GetModel(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldShow = "*";
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_ServicePanel> GetList(string strWhere, string strFieldOrder, int PageSize, int page, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as List<Lebi_ServicePanel>;
		   }
		   ...
		   if (cachekey != "" && list.Count > 0)
		         LB.DataAccess.DB.Instance.SetMemchche(cachekey, list, "Lebi_ServicePanel", 0 , cachestr , seconds);
		   return list;
		}
		public List<Lebi_ServicePanel> GetList(SQLPara para, int PageSize, int page, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldKey = "id";
		   string strFieldShow = "*";
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_ServicePanel> GetList(string strWhere,string strFieldOrder, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
		...
		public List<Lebi_ServicePanel> GetList(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   StringBuilder strSql=new StringBuilder();
		...
		public Lebi_ServicePanel BindForm(Lebi_ServicePanel model)
		{
			if (HttpContext.Current.Request["Account"] != null)
				model.Account=LB.Tools.RequestTool.RequestString("Account");
			if (HttpContext.Current.Request["Face"] != null)
				model.Face=LB.Tools.RequestTool.RequestInt("Face",0);
			...
			if (HttpContext.Current.Request["ServicePanel_Group_id"] != null)
				model.ServicePanel_Group_id=LB.Tools.RequestTool.RequestInt("ServicePanel_Group_id",0);
			if (HttpContext.Current.Request["ServicePanel_Type_id"] != null)
				model.ServicePanel_Type_id=LB.Tools.RequestTool.RequestInt("ServicePanel_Type_id",0);
		...
		{
			if (HttpContext.Current.Request["Account"] != null)
				model.Account=LB.Tools.RequestTool.RequestSafeString("Account");
			if (HttpContext.Current.Request["Face"] != null)
				model.Face=LB.Tools.RequestTool.RequestInt("Face",0);
			if (HttpContext.Current.Request["Language"] != null)
		...
		public Lebi_ServicePanel ReaderBind(IDataReader dataReader)
		{
			...
			if(ojb != null && ojb != DBNull.Value)
			{
				model.Face= Convert.ToInt32(ojb);
			}
			...
			if(ojb != null && ojb != DBNull.Value)
			{
				model.ServicePanel_Group_id= Convert.ToInt32(ojb);
			}
			ojb = dataReader["ServicePanel_Type_id"];
			if(ojb != null && ojb != DBNull.Value)
			{
				model.ServicePanel_Type_id= Convert.ToInt32(ojb);
			}
			ojb = dataReader["Sort"];
			if(ojb != null && ojb != DBNull.Value)
			{
