...
namespace DB.LebiShop
{
	...
	/// </summary>
	public partial class D_Lebi_Supplier_ProPerty
	{
		static D_Lebi_Supplier_ProPerty _Instance;
		public static D_Lebi_Supplier_ProPerty Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            _Instance = new D_Lebi_Supplier_ProPerty("Lebi_Supplier_ProPerty");
		        }
		        return _Instance;
		    }
		    set
		...
		string TableName = "Lebi_Supplier_ProPerty";
		public D_Lebi_Supplier_ProPerty(string tablename)
		{
		    TableName = tablename;
		}
		...
		public string GetValue(string colName,string strWhere,int seconds=0)
		{
		   ...
		   try
		   {
		       ...
		       if (cachekey != "")
		           LB.DataAccess.DB.Instance.SetMemchche(cachekey, val, "Lebi_Supplier_ProPerty", 0 , cachestr , seconds);
		   }
		   catch
		   {
		       val = "";
		...
		public int Add(Lebi_Supplier_ProPerty model)
		{
			...
			strSql.Append(LB.DataAccess.DB.BaseUtilsInstance.ColName("Pro_Type_id")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("ProPerty")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Supplier_id")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Type_id_ProPertyType")+")");
			strSql.Append(" values (");
			strSql.Append("@Pro_Type_id,@ProPerty,@Supplier_id,@Type_id_ProPertyType);select @@identity;");
		...
		public void Update(Lebi_Supplier_ProPerty model)
		{
			...
			if((","+model.UpdateCols+",").IndexOf(",ProPerty,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("ProPerty")+"= @ProPerty");
		...
		/// 得到一个对象实体 by id
		/// </summary>
		public Lebi_Supplier_ProPerty GetModel(int id, int seconds=0)
		{
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as Lebi_Supplier_ProPerty;
		   }
		   Lebi_Supplier_ProPerty model = null;
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow,  strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           ...
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Supplier_ProPerty",id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by where条件
		/// </summary>
		public Lebi_Supplier_ProPerty GetModel(string strWhere, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, "", "");
		   ...
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow, strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           model = ReaderBind(dataReader);
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Supplier_ProPerty",model.id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_Supplier_ProPerty GetModel(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldShow = "*";
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_Supplier_ProPerty> GetList(string strWhere, string strFieldOrder, int PageSize, int page, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as List<Lebi_Supplier_ProPerty>;
		   }
		   ...
		   if (cachekey != "" && list.Count > 0)
		         LB.DataAccess.DB.Instance.SetMemchche(cachekey, list, "Lebi_Supplier_ProPerty", 0 , cachestr , seconds);
		   return list;
		}
		public List<Lebi_Supplier_ProPerty> GetList(SQLPara para, int PageSize, int page, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldKey = "id";
		   string strFieldShow = "*";
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_Supplier_ProPerty> GetList(string strWhere,string strFieldOrder, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
		...
		public List<Lebi_Supplier_ProPerty> GetList(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   StringBuilder strSql=new StringBuilder();
		...
		public Lebi_Supplier_ProPerty BindForm(Lebi_Supplier_ProPerty model)
		{
			...
			if (HttpContext.Current.Request["ProPerty"] != null)
				model.ProPerty=LB.Tools.RequestTool.RequestString("ProPerty");
		...
		public Lebi_Supplier_ProPerty SafeBindForm(Lebi_Supplier_ProPerty model)
		{
			if (HttpContext.Current.Request["Pro_Type_id"] != null)
				model.Pro_Type_id=LB.Tools.RequestTool.RequestInt("Pro_Type_id",0);
			if (HttpContext.Current.Request["ProPerty"] != null)
				model.ProPerty=LB.Tools.RequestTool.RequestSafeString("ProPerty");
			if (HttpContext.Current.Request["Supplier_id"] != null)
				model.Supplier_id=LB.Tools.RequestTool.RequestInt("Supplier_id",0);
			if (HttpContext.Current.Request["Type_id_ProPertyType"] != null)
				model.Type_id_ProPertyType=LB.Tools.RequestTool.RequestInt("Type_id_ProPertyType",0);
