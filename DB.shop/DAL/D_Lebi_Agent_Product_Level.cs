...
namespace DB.LebiShop
{
	...
	/// </summary>
	public partial class D_Lebi_Agent_Product_Level
	{
		static D_Lebi_Agent_Product_Level _Instance;
		public static D_Lebi_Agent_Product_Level Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            _Instance = new D_Lebi_Agent_Product_Level("Lebi_Agent_Product_Level");
		        }
		        return _Instance;
		    }
		    set
		...
		string TableName = "Lebi_Agent_Product_Level";
		public D_Lebi_Agent_Product_Level(string tablename)
		{
		    TableName = tablename;
		}
		...
		public string GetValue(string colName,string strWhere,int seconds=0)
		{
		   ...
		   try
		   {
		       ...
		       if (cachekey != "")
		           LB.DataAccess.DB.Instance.SetMemchche(cachekey, val, "Lebi_Agent_Product_Level", 0 , cachestr , seconds);
		   }
		   catch
		   {
		       val = "";
		...
		public int Add(Lebi_Agent_Product_Level model)
		{
			...
			strSql.Append(LB.DataAccess.DB.BaseUtilsInstance.ColName("CardOrder_id")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Commission")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Content")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Count_Product")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Count_product_change")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Name")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Price")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Sort")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Years")+")");
			strSql.Append(" values (");
			strSql.Append("@CardOrder_id,@Commission,@Content,@Count_Product,@Count_product_change,@Name,@Price,@Sort,@Years);select @@identity;");
		...
		public void Update(Lebi_Agent_Product_Level model)
		{
			...
			if((","+model.UpdateCols+",").IndexOf(",Years,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("Years")+"= @Years");
		...
		/// 得到一个对象实体 by id
		/// </summary>
		public Lebi_Agent_Product_Level GetModel(int id, int seconds=0)
		{
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as Lebi_Agent_Product_Level;
		   }
		   Lebi_Agent_Product_Level model = null;
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow,  strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           ...
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Agent_Product_Level",id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by where条件
		/// </summary>
		public Lebi_Agent_Product_Level GetModel(string strWhere, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, "", "");
		   ...
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow, strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           model = ReaderBind(dataReader);
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Agent_Product_Level",model.id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_Agent_Product_Level GetModel(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldShow = "*";
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_Agent_Product_Level> GetList(string strWhere, string strFieldOrder, int PageSize, int page, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as List<Lebi_Agent_Product_Level>;
		   }
		   ...
		   if (cachekey != "" && list.Count > 0)
		         LB.DataAccess.DB.Instance.SetMemchche(cachekey, list, "Lebi_Agent_Product_Level", 0 , cachestr , seconds);
		   return list;
		}
		public List<Lebi_Agent_Product_Level> GetList(SQLPara para, int PageSize, int page, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldKey = "id";
		   string strFieldShow = "*";
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_Agent_Product_Level> GetList(string strWhere,string strFieldOrder, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
		...
		public List<Lebi_Agent_Product_Level> GetList(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   StringBuilder strSql=new StringBuilder();
		...
		public Lebi_Agent_Product_Level BindForm(Lebi_Agent_Product_Level model)
		{
			...
			if (HttpContext.Current.Request["Years"] != null)
				model.Years=LB.Tools.RequestTool.RequestInt("Years",0);
				return model;
		}
		/// <summary>
		/// 安全方式绑定对象表单
		...
		public Lebi_Agent_Product_Level ReaderBind(IDataReader dataReader)
		{
			...
			if(ojb != null && ojb != DBNull.Value)
			{
				model.Years= Convert.ToInt32(ojb);
			}
			return model;
		}
