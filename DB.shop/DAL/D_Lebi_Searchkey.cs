...
{
	/// <summary>
	/// 数据访问类D_Lebi_Searchkey。
	/// </summary>
	public partial class D_Lebi_Searchkey
	{
		static D_Lebi_Searchkey _Instance;
		public static D_Lebi_Searchkey Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            _Instance = new D_Lebi_Searchkey("Lebi_Searchkey");
		        }
		        return _Instance;
		    }
		    set
		...
		string TableName = "Lebi_Searchkey";
		public D_Lebi_Searchkey(string tablename)
		{
		    TableName = tablename;
		}
		...
		public string GetValue(string colName,string strWhere,int seconds=0)
		{
		   ...
		   try
		   {
		       ...
		       if (cachekey != "")
		           LB.DataAccess.DB.Instance.SetMemchche(cachekey, val, "Lebi_Searchkey", 0 , cachestr , seconds);
		   }
		   catch
		   {
		       val = "";
		...
		public int Add(Lebi_Searchkey model)
		{
			...
			strSql.Append(LB.DataAccess.DB.BaseUtilsInstance.ColName("Language_Code")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Language_id")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Name")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Sort")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Type")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("URL")+")");
			strSql.Append(" values (");
			strSql.Append("@Language_Code,@Language_id,@Name,@Sort,@Type,@URL);select @@identity;");
		...
		public void Update(Lebi_Searchkey model)
		{
			...
			if((","+model.UpdateCols+",").IndexOf(",Language_id,")>-1 || model.UpdateCols=="")
		...
		/// 得到一个对象实体 by id
		/// </summary>
		public Lebi_Searchkey GetModel(int id, int seconds=0)
		{
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as Lebi_Searchkey;
		   }
		   Lebi_Searchkey model = null;
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow,  strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           ...
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Searchkey",id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by where条件
		/// </summary>
		public Lebi_Searchkey GetModel(string strWhere, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, "", "");
		   ...
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow, strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           model = ReaderBind(dataReader);
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Searchkey",model.id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_Searchkey GetModel(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldShow = "*";
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_Searchkey> GetList(string strWhere, string strFieldOrder, int PageSize, int page, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as List<Lebi_Searchkey>;
		   }
		   List<Lebi_Searchkey> list = new List<Lebi_Searchkey>();
		   ...
		   if (cachekey != "" && list.Count > 0)
		         LB.DataAccess.DB.Instance.SetMemchche(cachekey, list, "Lebi_Searchkey", 0 , cachestr , seconds);
		   return list;
		}
		public List<Lebi_Searchkey> GetList(SQLPara para, int PageSize, int page, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldKey = "id";
		   string strFieldShow = "*";
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_Searchkey> GetList(string strWhere,string strFieldOrder, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
		...
		public List<Lebi_Searchkey> GetList(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   StringBuilder strSql=new StringBuilder();
		...
		/// 绑定对象表单
		/// </summary>
		public Lebi_Searchkey BindForm(Lebi_Searchkey model)
		{
			...
			if (HttpContext.Current.Request["Type"] != null)
				model.Type=LB.Tools.RequestTool.RequestInt("Type",0);
