...
namespace DB.LebiShop
{
	...
	/// 数据访问类D_Lebi_Menu。
	/// </summary>
	public partial class D_Lebi_Menu
	{
		static D_Lebi_Menu _Instance;
		public static D_Lebi_Menu Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            _Instance = new D_Lebi_Menu("Lebi_Menu");
		        }
		        return _Instance;
		    }
		    set
		...
		string TableName = "Lebi_Menu";
		public D_Lebi_Menu(string tablename)
		{
		    TableName = tablename;
		}
		...
		public string GetValue(string colName,string strWhere,int seconds=0)
		{
		   ...
		   try
		   {
		       ...
		       if (cachekey != "")
		           LB.DataAccess.DB.Instance.SetMemchche(cachekey, val, "Lebi_Menu", 0 , cachestr , seconds);
		   }
		   catch
		   {
		       val = "";
		...
		public int Add(Lebi_Menu model)
		{
			...
			strSql.Append(LB.DataAccess.DB.BaseUtilsInstance.ColName("Code")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Image")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Isshow")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("IsSYS")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Name")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("parentCode")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("parentid")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Sort")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("URL")+")");
			strSql.Append(" values (");
			strSql.Append("@Code,@Image,@Isshow,@IsSYS,@Name,@parentCode,@parentid,@Sort,@URL);select @@identity;");
		...
		public void Update(Lebi_Menu model)
		{
			...
			if((","+model.UpdateCols+",").IndexOf(",parentCode,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("parentCode")+"= @parentCode");
		...
		/// 得到一个对象实体 by id
		/// </summary>
		public Lebi_Menu GetModel(int id, int seconds=0)
		{
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as Lebi_Menu;
		   }
		   Lebi_Menu model = null;
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow,  strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           ...
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Menu",id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by where条件
		/// </summary>
		public Lebi_Menu GetModel(string strWhere, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, "", "");
		   ...
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow, strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           model = ReaderBind(dataReader);
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Menu",model.id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_Menu GetModel(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldShow = "*";
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_Menu> GetList(string strWhere, string strFieldOrder, int PageSize, int page, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		   ...
		   if (cachekey != "" && list.Count > 0)
		         LB.DataAccess.DB.Instance.SetMemchche(cachekey, list, "Lebi_Menu", 0 , cachestr , seconds);
		   return list;
		}
		public List<Lebi_Menu> GetList(SQLPara para, int PageSize, int page, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldKey = "id";
		   string strFieldShow = "*";
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_Menu> GetList(string strWhere,string strFieldOrder, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
		...
		public List<Lebi_Menu> GetList(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   StringBuilder strSql=new StringBuilder();
		...
		public Lebi_Menu BindForm(Lebi_Menu model)
		{
			...
			if (HttpContext.Current.Request["parentCode"] != null)
				model.parentCode=LB.Tools.RequestTool.RequestString("parentCode");
		...
		public Lebi_Menu SafeBindForm(Lebi_Menu model)
		{
			...
			if (HttpContext.Current.Request["parentCode"] != null)
				model.parentCode=LB.Tools.RequestTool.RequestSafeString("parentCode");
			if (HttpContext.Current.Request["parentid"] != null)
				model.parentid=LB.Tools.RequestTool.RequestInt("parentid",0);
			if (HttpContext.Current.Request["Sort"] != null)
				model.Sort=LB.Tools.RequestTool.RequestInt("Sort",0);
