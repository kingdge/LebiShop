...
{
	/// <summary>
	/// 数据访问类D_Lebi_OnlinePay_Log。
	/// </summary>
	public partial class D_Lebi_OnlinePay_Log
	{
		static D_Lebi_OnlinePay_Log _Instance;
		public static D_Lebi_OnlinePay_Log Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            _Instance = new D_Lebi_OnlinePay_Log("Lebi_OnlinePay_Log");
		        }
		        return _Instance;
		    }
		    set
		...
		string TableName = "Lebi_OnlinePay_Log";
		public D_Lebi_OnlinePay_Log(string tablename)
		{
		    TableName = tablename;
		}
		...
		public string GetValue(string colName,string strWhere,int seconds=0)
		{
		   ...
		   try
		   {
		       ...
		       if (cachekey != "")
		           LB.DataAccess.DB.Instance.SetMemchche(cachekey, val, "Lebi_OnlinePay_Log", 0 , cachestr , seconds);
		   }
		   catch
		   {
		       val = "";
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Lebi_OnlinePay_Log model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into "+ TableName + " (");
			strSql.Append(LB.DataAccess.DB.BaseUtilsInstance.ColName("Order_id")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("OnlinePay_id")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Time_add")+")");
			strSql.Append(" values (");
			strSql.Append("@Order_id,@OnlinePay_id,@Time_add);select @@identity;");
		...
		/// 更新一条数据
		/// </summary>
		public void Update(Lebi_OnlinePay_Log model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 得到一个对象实体 by id
		/// </summary>
		public Lebi_OnlinePay_Log GetModel(int id, int seconds=0)
		{
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as Lebi_OnlinePay_Log;
		   }
		   Lebi_OnlinePay_Log model = null;
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow,  strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           ...
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_OnlinePay_Log",id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by where条件
		/// </summary>
		public Lebi_OnlinePay_Log GetModel(string strWhere, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, "", "");
		   ...
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow, strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           model = ReaderBind(dataReader);
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_OnlinePay_Log",model.id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_OnlinePay_Log GetModel(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldShow = "*";
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_OnlinePay_Log> GetList(string strWhere, string strFieldOrder, int PageSize, int page, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as List<Lebi_OnlinePay_Log>;
		   }
		   List<Lebi_OnlinePay_Log> list = new List<Lebi_OnlinePay_Log>();
		   ...
		   if (cachekey != "" && list.Count > 0)
		         LB.DataAccess.DB.Instance.SetMemchche(cachekey, list, "Lebi_OnlinePay_Log", 0 , cachestr , seconds);
		   return list;
		}
		public List<Lebi_OnlinePay_Log> GetList(SQLPara para, int PageSize, int page, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldKey = "id";
		   string strFieldShow = "*";
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_OnlinePay_Log> GetList(string strWhere,string strFieldOrder, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
		...
		public List<Lebi_OnlinePay_Log> GetList(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   StringBuilder strSql=new StringBuilder();
		...
		/// 绑定对象表单
		/// </summary>
		public Lebi_OnlinePay_Log BindForm(Lebi_OnlinePay_Log model)
		{
		...
		/// 安全方式绑定对象表单
		/// </summary>
		public Lebi_OnlinePay_Log SafeBindForm(Lebi_OnlinePay_Log model)
		{
			if (HttpContext.Current.Request["Order_id"] != null)
				model.Order_id=LB.Tools.RequestTool.RequestInt("Order_id",0);
			if (HttpContext.Current.Request["OnlinePay_id"] != null)
		...
		/// 对象实体绑定数据
		/// </summary>
		public Lebi_OnlinePay_Log ReaderBind(IDataReader dataReader)
		{
			Lebi_OnlinePay_Log model=new Lebi_OnlinePay_Log();
			object ojb; 
			ojb = dataReader["id"];
