...
namespace DB.LebiShop
{
	...
	/// </summary>
	public partial class D_Lebi_DT_Agent
	{
		static D_Lebi_DT_Agent _Instance;
		public static D_Lebi_DT_Agent Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            _Instance = new D_Lebi_DT_Agent("Lebi_DT_Agent");
		        }
		        return _Instance;
		    }
		    set
		...
		string TableName = "Lebi_DT_Agent";
		public D_Lebi_DT_Agent(string tablename)
		{
		    TableName = tablename;
		}
		...
		public string GetValue(string colName,string strWhere,int seconds=0)
		{
		   ...
		   try
		   {
		       ...
		       if (cachekey != "")
		           LB.DataAccess.DB.Instance.SetMemchche(cachekey, val, "Lebi_DT_Agent", 0 , cachestr , seconds);
		   }
		   catch
		   {
		       val = "";
		...
		public int Add(Lebi_DT_Agent model)
		{
			...
			strSql.Append(LB.DataAccess.DB.BaseUtilsInstance.ColName("Angent1_Commission")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Angent2_Commission")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Angent3_Commission")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("CommissionMoneyDays")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("DT_id")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("IsUsedAgent")+")");
			strSql.Append(" values (");
			strSql.Append("@Angent1_Commission,@Angent2_Commission,@Angent3_Commission,@CommissionMoneyDays,@DT_id,@IsUsedAgent);select @@identity;");
		...
		public void Update(Lebi_DT_Agent model)
		{
			...
			if((","+model.UpdateCols+",").IndexOf(",Angent1_Commission,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("Angent1_Commission")+"= @Angent1_Commission");
			if((","+model.UpdateCols+",").IndexOf(",Angent2_Commission,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("Angent2_Commission")+"= @Angent2_Commission");
			if((","+model.UpdateCols+",").IndexOf(",Angent3_Commission,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("Angent3_Commission")+"= @Angent3_Commission");
			if((","+model.UpdateCols+",").IndexOf(",CommissionMoneyDays,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("CommissionMoneyDays")+"= @CommissionMoneyDays");
		...
		/// 得到一个对象实体 by id
		/// </summary>
		public Lebi_DT_Agent GetModel(int id, int seconds=0)
		{
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as Lebi_DT_Agent;
		   }
		   Lebi_DT_Agent model = null;
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow,  strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           ...
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_DT_Agent",id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by where条件
		/// </summary>
		public Lebi_DT_Agent GetModel(string strWhere, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, "", "");
		   ...
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow, strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           model = ReaderBind(dataReader);
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_DT_Agent",model.id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_DT_Agent GetModel(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldShow = "*";
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_DT_Agent> GetList(string strWhere, string strFieldOrder, int PageSize, int page, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as List<Lebi_DT_Agent>;
		   }
		   ...
		   if (cachekey != "" && list.Count > 0)
		         LB.DataAccess.DB.Instance.SetMemchche(cachekey, list, "Lebi_DT_Agent", 0 , cachestr , seconds);
		   return list;
		}
		public List<Lebi_DT_Agent> GetList(SQLPara para, int PageSize, int page, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldKey = "id";
		   string strFieldShow = "*";
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_DT_Agent> GetList(string strWhere,string strFieldOrder, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
		...
		public List<Lebi_DT_Agent> GetList(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   StringBuilder strSql=new StringBuilder();
		...
		public Lebi_DT_Agent BindForm(Lebi_DT_Agent model)
		{
			if (HttpContext.Current.Request["Angent1_Commission"] != null)
				model.Angent1_Commission=LB.Tools.RequestTool.RequestDecimal("Angent1_Commission",0);
			if (HttpContext.Current.Request["Angent2_Commission"] != null)
				model.Angent2_Commission=LB.Tools.RequestTool.RequestDecimal("Angent2_Commission",0);
			if (HttpContext.Current.Request["Angent3_Commission"] != null)
				model.Angent3_Commission=LB.Tools.RequestTool.RequestDecimal("Angent3_Commission",0);
			if (HttpContext.Current.Request["CommissionMoneyDays"] != null)
				model.CommissionMoneyDays=LB.Tools.RequestTool.RequestInt("CommissionMoneyDays",0);
		...
		public Lebi_DT_Agent ReaderBind(IDataReader dataReader)
		{
			...
			if(ojb != null && ojb != DBNull.Value)
			{
				model.CommissionMoneyDays= Convert.ToInt32(ojb);
			}
