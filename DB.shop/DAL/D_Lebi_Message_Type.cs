...
{
	/// <summary>
	/// 数据访问类D_Lebi_Message_Type。
	/// </summary>
	public partial class D_Lebi_Message_Type
	{
		static D_Lebi_Message_Type _Instance;
		public static D_Lebi_Message_Type Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            _Instance = new D_Lebi_Message_Type("Lebi_Message_Type");
		        }
		        return _Instance;
		    }
		    set
		...
		string TableName = "Lebi_Message_Type";
		public D_Lebi_Message_Type(string tablename)
		{
		    TableName = tablename;
		}
		...
		public string GetValue(string colName,string strWhere,int seconds=0)
		{
		   ...
		   try
		   {
		       ...
		       if (cachekey != "")
		           LB.DataAccess.DB.Instance.SetMemchche(cachekey, val, "Lebi_Message_Type", 0 , cachestr , seconds);
		   }
		   catch
		   {
		       val = "";
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Lebi_Message_Type model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into "+ TableName + " (");
			strSql.Append(LB.DataAccess.DB.BaseUtilsInstance.ColName("Name")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Sort")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Type_id_MessageTypeClass")+")");
			strSql.Append(" values (");
			strSql.Append("@Name,@Sort,@Type_id_MessageTypeClass);select @@identity;");
			SqlParameter[] parameters = {
					new SqlParameter("@Name", model.Name),
					new SqlParameter("@Sort", model.Sort),
					new SqlParameter("@Type_id_MessageTypeClass", model.Type_id_MessageTypeClass)};
		...
		/// 更新一条数据
		/// </summary>
		public void Update(Lebi_Message_Type model)
		{
			StringBuilder strSql=new StringBuilder();
			...
			if((","+model.UpdateCols+",").IndexOf(",Type_id_MessageTypeClass,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("Type_id_MessageTypeClass")+"= @Type_id_MessageTypeClass");
		...
		/// 得到一个对象实体 by id
		/// </summary>
		public Lebi_Message_Type GetModel(int id, int seconds=0)
		{
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as Lebi_Message_Type;
		   }
		   Lebi_Message_Type model = null;
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow,  strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           ...
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Message_Type",id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by where条件
		/// </summary>
		public Lebi_Message_Type GetModel(string strWhere, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, "", "");
		   ...
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow, strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           model = ReaderBind(dataReader);
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Message_Type",model.id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_Message_Type GetModel(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldShow = "*";
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_Message_Type> GetList(string strWhere, string strFieldOrder, int PageSize, int page, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as List<Lebi_Message_Type>;
		   }
		   List<Lebi_Message_Type> list = new List<Lebi_Message_Type>();
		   ...
		   if (cachekey != "" && list.Count > 0)
		         LB.DataAccess.DB.Instance.SetMemchche(cachekey, list, "Lebi_Message_Type", 0 , cachestr , seconds);
		   return list;
		}
		public List<Lebi_Message_Type> GetList(SQLPara para, int PageSize, int page, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldKey = "id";
		   string strFieldShow = "*";
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_Message_Type> GetList(string strWhere,string strFieldOrder, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
		...
		public List<Lebi_Message_Type> GetList(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   StringBuilder strSql=new StringBuilder();
		...
		/// 绑定对象表单
		/// </summary>
		public Lebi_Message_Type BindForm(Lebi_Message_Type model)
		{
			if (HttpContext.Current.Request["Name"] != null)
				model.Name=LB.Tools.RequestTool.RequestString("Name");
			if (HttpContext.Current.Request["Sort"] != null)
				model.Sort=LB.Tools.RequestTool.RequestInt("Sort",0);
			if (HttpContext.Current.Request["Type_id_MessageTypeClass"] != null)
				model.Type_id_MessageTypeClass=LB.Tools.RequestTool.RequestInt("Type_id_MessageTypeClass",0);
				return model;
		}
		/// <summary>
		/// 安全方式绑定对象表单
		/// </summary>
		public Lebi_Message_Type SafeBindForm(Lebi_Message_Type model)
		{
			if (HttpContext.Current.Request["Name"] != null)
		...
		/// 对象实体绑定数据
		/// </summary>
		public Lebi_Message_Type ReaderBind(IDataReader dataReader)
		{
			Lebi_Message_Type model=new Lebi_Message_Type();
			object ojb; 
			ojb = dataReader["id"];
			...
			ojb = dataReader["Type_id_MessageTypeClass"];
			if(ojb != null && ojb != DBNull.Value)
			{
				model.Type_id_MessageTypeClass= Convert.ToInt32(ojb);
			}
			return model;
		}
