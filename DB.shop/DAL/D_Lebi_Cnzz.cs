...
namespace DB.LebiShop
{
	...
	/// 数据访问类D_Lebi_Cnzz。
	/// </summary>
	public partial class D_Lebi_Cnzz
	{
		static D_Lebi_Cnzz _Instance;
		public static D_Lebi_Cnzz Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            _Instance = new D_Lebi_Cnzz("Lebi_Cnzz");
		        }
		        return _Instance;
		    }
		    set
		...
		string TableName = "Lebi_Cnzz";
		public D_Lebi_Cnzz(string tablename)
		{
		    TableName = tablename;
		}
		...
		public string GetValue(string colName,string strWhere,int seconds=0)
		{
		   ...
		   try
		   {
		       ...
		       if (cachekey != "")
		           LB.DataAccess.DB.Instance.SetMemchche(cachekey, val, "Lebi_Cnzz", 0 , cachestr , seconds);
		   }
		   catch
		   {
		       val = "";
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Lebi_Cnzz model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into "+ TableName + " (");
			strSql.Append(LB.DataAccess.DB.BaseUtilsInstance.ColName("Ccontent")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("state")+")");
			strSql.Append(" values (");
			strSql.Append("@Ccontent,@state);select @@identity;");
		...
		/// 更新一条数据
		/// </summary>
		public void Update(Lebi_Cnzz model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update "+ TableName + " set ");
			List<string> cols = new List<string>();
			if((","+model.UpdateCols+",").IndexOf(",Ccontent,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("Ccontent")+"= @Ccontent");
			if((","+model.UpdateCols+",").IndexOf(",state,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("state")+"= @state");
		...
		/// 得到一个对象实体 by id
		/// </summary>
		public Lebi_Cnzz GetModel(int id, int seconds=0)
		{
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as Lebi_Cnzz;
		   }
		   Lebi_Cnzz model = null;
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow,  strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           ...
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Cnzz",id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by where条件
		/// </summary>
		public Lebi_Cnzz GetModel(string strWhere, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, "", "");
		   ...
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow, strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           model = ReaderBind(dataReader);
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Cnzz",model.id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_Cnzz GetModel(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldShow = "*";
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_Cnzz> GetList(string strWhere, string strFieldOrder, int PageSize, int page, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as List<Lebi_Cnzz>;
		   }
		   List<Lebi_Cnzz> list = new List<Lebi_Cnzz>();
		   ...
		   if (cachekey != "" && list.Count > 0)
		         LB.DataAccess.DB.Instance.SetMemchche(cachekey, list, "Lebi_Cnzz", 0 , cachestr , seconds);
		   return list;
		}
		public List<Lebi_Cnzz> GetList(SQLPara para, int PageSize, int page, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldKey = "id";
		   string strFieldShow = "*";
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_Cnzz> GetList(string strWhere,string strFieldOrder, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
		...
		public List<Lebi_Cnzz> GetList(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   StringBuilder strSql=new StringBuilder();
		...
		/// 绑定对象表单
		/// </summary>
		public Lebi_Cnzz BindForm(Lebi_Cnzz model)
		{
			if (HttpContext.Current.Request["Ccontent"] != null)
				model.Ccontent=LB.Tools.RequestTool.RequestString("Ccontent");
			if (HttpContext.Current.Request["state"] != null)
				model.state=LB.Tools.RequestTool.RequestInt("state",0);
				return model;
		}
		/// <summary>
		/// 安全方式绑定对象表单
		/// </summary>
		public Lebi_Cnzz SafeBindForm(Lebi_Cnzz model)
		{
			if (HttpContext.Current.Request["Ccontent"] != null)
				model.Ccontent=LB.Tools.RequestTool.RequestSafeString("Ccontent");
			if (HttpContext.Current.Request["state"] != null)
				model.state=LB.Tools.RequestTool.RequestInt("state",0);
				return model;
		}
		...
		/// 对象实体绑定数据
		/// </summary>
		public Lebi_Cnzz ReaderBind(IDataReader dataReader)
		{
			Lebi_Cnzz model=new Lebi_Cnzz();
			object ojb; 
			ojb = dataReader["id"];
			...
			model.Ccontent=dataReader["Ccontent"].ToString();
			ojb = dataReader["state"];
			if(ojb != null && ojb != DBNull.Value)
			{
				model.state= Convert.ToInt32(ojb);
			}
			return model;
		}
