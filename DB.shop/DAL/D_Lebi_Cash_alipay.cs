...
namespace DB.LebiShop
{
	...
	/// </summary>
	public partial class D_Lebi_Cash_alipay
	{
		static D_Lebi_Cash_alipay _Instance;
		public static D_Lebi_Cash_alipay Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            _Instance = new D_Lebi_Cash_alipay("Lebi_Cash_alipay");
		        }
		        return _Instance;
		    }
		    set
		...
		string TableName = "Lebi_Cash_alipay";
		public D_Lebi_Cash_alipay(string tablename)
		{
		    TableName = tablename;
		}
		...
		public string GetValue(string colName,string strWhere,int seconds=0)
		{
		   ...
		   try
		   {
		       ...
		       if (cachekey != "")
		           LB.DataAccess.DB.Instance.SetMemchche(cachekey, val, "Lebi_Cash_alipay", 0 , cachestr , seconds);
		   }
		   catch
		   {
		       val = "";
		...
		public int Add(Lebi_Cash_alipay model)
		{
			...
			strSql.Append(LB.DataAccess.DB.BaseUtilsInstance.ColName("alipay_user")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("alipay_username")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Cash_ids")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Code")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Content")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("count")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("IsPaid")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Money")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("result_no")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("result_yes")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Time_Add")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Time_Paid")+")");
			strSql.Append(" values (");
			strSql.Append("@alipay_user,@alipay_username,@Cash_ids,@Code,@Content,@count,@IsPaid,@Money,@result_no,@result_yes,@Time_Add,@Time_Paid);select @@identity;");
		...
		public void Update(Lebi_Cash_alipay model)
		{
			...
			if((","+model.UpdateCols+",").IndexOf(",alipay_user,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("alipay_user")+"= @alipay_user");
			if((","+model.UpdateCols+",").IndexOf(",alipay_username,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("alipay_username")+"= @alipay_username");
			if((","+model.UpdateCols+",").IndexOf(",Cash_ids,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("Cash_ids")+"= @Cash_ids");
			...
			if((","+model.UpdateCols+",").IndexOf(",result_no,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("result_no")+"= @result_no");
			if((","+model.UpdateCols+",").IndexOf(",result_yes,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("result_yes")+"= @result_yes");
		...
		/// 得到一个对象实体 by id
		/// </summary>
		public Lebi_Cash_alipay GetModel(int id, int seconds=0)
		{
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as Lebi_Cash_alipay;
		   }
		   Lebi_Cash_alipay model = null;
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow,  strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           ...
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Cash_alipay",id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by where条件
		/// </summary>
		public Lebi_Cash_alipay GetModel(string strWhere, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, "", "");
		   ...
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow, strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           model = ReaderBind(dataReader);
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Cash_alipay",model.id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_Cash_alipay GetModel(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldShow = "*";
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_Cash_alipay> GetList(string strWhere, string strFieldOrder, int PageSize, int page, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as List<Lebi_Cash_alipay>;
		   }
		   ...
		   if (cachekey != "" && list.Count > 0)
		         LB.DataAccess.DB.Instance.SetMemchche(cachekey, list, "Lebi_Cash_alipay", 0 , cachestr , seconds);
		   return list;
		}
		public List<Lebi_Cash_alipay> GetList(SQLPara para, int PageSize, int page, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldKey = "id";
		   string strFieldShow = "*";
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_Cash_alipay> GetList(string strWhere,string strFieldOrder, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
		...
		public List<Lebi_Cash_alipay> GetList(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   StringBuilder strSql=new StringBuilder();
		...
		public Lebi_Cash_alipay BindForm(Lebi_Cash_alipay model)
		{
			if (HttpContext.Current.Request["alipay_user"] != null)
				model.alipay_user=LB.Tools.RequestTool.RequestString("alipay_user");
			if (HttpContext.Current.Request["alipay_username"] != null)
				model.alipay_username=LB.Tools.RequestTool.RequestString("alipay_username");
			if (HttpContext.Current.Request["Cash_ids"] != null)
				model.Cash_ids=LB.Tools.RequestTool.RequestString("Cash_ids");
			...
			if (HttpContext.Current.Request["result_no"] != null)
				model.result_no=LB.Tools.RequestTool.RequestString("result_no");
			if (HttpContext.Current.Request["result_yes"] != null)
				model.result_yes=LB.Tools.RequestTool.RequestString("result_yes");
		...
		{
			if (HttpContext.Current.Request["alipay_user"] != null)
				model.alipay_user=LB.Tools.RequestTool.RequestSafeString("alipay_user");
			if (HttpContext.Current.Request["alipay_username"] != null)
				model.alipay_username=LB.Tools.RequestTool.RequestSafeString("alipay_username");
			if (HttpContext.Current.Request["Cash_ids"] != null)
				model.Cash_ids=LB.Tools.RequestTool.RequestSafeString("Cash_ids");
			if (HttpContext.Current.Request["Code"] != null)
			...
			if (HttpContext.Current.Request["Money"] != null)
				model.Money=LB.Tools.RequestTool.RequestDecimal("Money",0);
			if (HttpContext.Current.Request["result_no"] != null)
				model.result_no=LB.Tools.RequestTool.RequestSafeString("result_no");
			if (HttpContext.Current.Request["result_yes"] != null)
				model.result_yes=LB.Tools.RequestTool.RequestSafeString("result_yes");
			if (HttpContext.Current.Request["Time_Add"] != null)
				model.Time_Add=LB.Tools.RequestTool.RequestTime("Time_Add", System.DateTime.Now);
			if (HttpContext.Current.Request["Time_Paid"] != null)
				model.Time_Paid=LB.Tools.RequestTool.RequestTime("Time_Paid", System.DateTime.Now);
