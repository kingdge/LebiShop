...
namespace DB.LebiShop
{
	...
	/// 数据访问类D_Lebi_Email。
	/// </summary>
	public partial class D_Lebi_Email
	{
		static D_Lebi_Email _Instance;
		public static D_Lebi_Email Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            _Instance = new D_Lebi_Email("Lebi_Email");
		        }
		        return _Instance;
		    }
		    set
		...
		string TableName = "Lebi_Email";
		public D_Lebi_Email(string tablename)
		{
		    TableName = tablename;
		}
		...
		public string GetValue(string colName,string strWhere,int seconds=0)
		{
		   ...
		   try
		   {
		       ...
		       if (cachekey != "")
		           LB.DataAccess.DB.Instance.SetMemchche(cachekey, val, "Lebi_Email", 0 , cachestr , seconds);
		   }
		   catch
		   {
		       val = "";
		...
		public int Add(Lebi_Email model)
		{
			...
			strSql.Append(LB.DataAccess.DB.BaseUtilsInstance.ColName("Content")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Count_send")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Email")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("EmailTask_id")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Keyid")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("TableName")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Time_Add")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Time_Send")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Time_Task")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Title")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Type_id_EmailStatus")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("User_id")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("User_Name")+")");
			strSql.Append(" values (");
			strSql.Append("@Content,@Count_send,@Email,@EmailTask_id,@Keyid,@TableName,@Time_Add,@Time_Send,@Time_Task,@Title,@Type_id_EmailStatus,@User_id,@User_Name);select @@identity;");
		...
		public void Update(Lebi_Email model)
		{
			...
			if((","+model.UpdateCols+",").IndexOf(",EmailTask_id,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("EmailTask_id")+"= @EmailTask_id");
			...
			if((","+model.UpdateCols+",").IndexOf(",Time_Send,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("Time_Send")+"= @Time_Send");
			...
			if((","+model.UpdateCols+",").IndexOf(",Type_id_EmailStatus,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("Type_id_EmailStatus")+"= @Type_id_EmailStatus");
			...
			if((","+model.UpdateCols+",").IndexOf(",User_Name,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("User_Name")+"= @User_Name");
		...
		/// 得到一个对象实体 by id
		/// </summary>
		public Lebi_Email GetModel(int id, int seconds=0)
		{
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as Lebi_Email;
		   }
		   Lebi_Email model = null;
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow,  strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           ...
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Email",id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by where条件
		/// </summary>
		public Lebi_Email GetModel(string strWhere, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, "", "");
		   ...
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow, strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           model = ReaderBind(dataReader);
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Email",model.id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_Email GetModel(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldShow = "*";
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_Email> GetList(string strWhere, string strFieldOrder, int PageSize, int page, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		   ...
		   if (cachekey != "" && list.Count > 0)
		         LB.DataAccess.DB.Instance.SetMemchche(cachekey, list, "Lebi_Email", 0 , cachestr , seconds);
		   return list;
		}
		public List<Lebi_Email> GetList(SQLPara para, int PageSize, int page, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldKey = "id";
		   string strFieldShow = "*";
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_Email> GetList(string strWhere,string strFieldOrder, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
		...
		public List<Lebi_Email> GetList(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   StringBuilder strSql=new StringBuilder();
		...
		public Lebi_Email BindForm(Lebi_Email model)
		{
			...
			if (HttpContext.Current.Request["EmailTask_id"] != null)
				model.EmailTask_id=LB.Tools.RequestTool.RequestInt("EmailTask_id",0);
			if (HttpContext.Current.Request["Keyid"] != null)
				model.Keyid=LB.Tools.RequestTool.RequestInt("Keyid",0);
			...
			if (HttpContext.Current.Request["Time_Send"] != null)
				model.Time_Send=LB.Tools.RequestTool.RequestTime("Time_Send", System.DateTime.Now);
			...
			if (HttpContext.Current.Request["Type_id_EmailStatus"] != null)
				model.Type_id_EmailStatus=LB.Tools.RequestTool.RequestInt("Type_id_EmailStatus",0);
			...
			if (HttpContext.Current.Request["User_Name"] != null)
				model.User_Name=LB.Tools.RequestTool.RequestString("User_Name");
				return model;
		}
		/// <summary>
		/// 安全方式绑定对象表单
		...
		public Lebi_Email SafeBindForm(Lebi_Email model)
		{
			...
			if (HttpContext.Current.Request["User_id"] != null)
				model.User_id=LB.Tools.RequestTool.RequestInt("User_id",0);
			if (HttpContext.Current.Request["User_Name"] != null)
				model.User_Name=LB.Tools.RequestTool.RequestSafeString("User_Name");
				return model;
		}
		...
		public Lebi_Email ReaderBind(IDataReader dataReader)
		{
			...
			if(ojb != null && ojb != DBNull.Value)
			{
				model.EmailTask_id= Convert.ToInt32(ojb);
			}
			ojb = dataReader["Keyid"];
			if(ojb != null && ojb != DBNull.Value)
			{
				model.Keyid= Convert.ToInt32(ojb);
			}
			...
			if(ojb != null && ojb != DBNull.Value)
			{
				model.Type_id_EmailStatus= Convert.ToInt32(ojb);
			}
