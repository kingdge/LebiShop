...
namespace DB.LebiShop
{
	...
	/// </summary>
	public partial class D_Lebi_ServicePanel_Type
	{
		static D_Lebi_ServicePanel_Type _Instance;
		public static D_Lebi_ServicePanel_Type Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            _Instance = new D_Lebi_ServicePanel_Type("Lebi_ServicePanel_Type");
		        }
		        return _Instance;
		    }
		    set
		...
		string TableName = "Lebi_ServicePanel_Type";
		public D_Lebi_ServicePanel_Type(string tablename)
		{
		    TableName = tablename;
		}
		...
		public string GetValue(string colName,string strWhere,int seconds=0)
		{
		   ...
		   try
		   {
		       ...
		       if (cachekey != "")
		           LB.DataAccess.DB.Instance.SetMemchche(cachekey, val, "Lebi_ServicePanel_Type", 0 , cachestr , seconds);
		   }
		   catch
		   {
		       val = "";
		...
		public int Add(Lebi_ServicePanel_Type model)
		{
			...
			strSql.Append(LB.DataAccess.DB.BaseUtilsInstance.ColName("Code")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Face")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("IsOnline")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Name")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Sort")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Text")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Url")+")");
			strSql.Append(" values (");
			strSql.Append("@Code,@Face,@IsOnline,@Name,@Sort,@Text,@Url);select @@identity;");
		...
		public void Update(Lebi_ServicePanel_Type model)
		{
			...
			if((","+model.UpdateCols+",").IndexOf(",IsOnline,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("IsOnline")+"= @IsOnline");
			...
			if((","+model.UpdateCols+",").IndexOf(",Text,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("Text")+"= @Text");
		...
		/// 得到一个对象实体 by id
		/// </summary>
		public Lebi_ServicePanel_Type GetModel(int id, int seconds=0)
		{
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as Lebi_ServicePanel_Type;
		   }
		   Lebi_ServicePanel_Type model = null;
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow,  strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           ...
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_ServicePanel_Type",id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by where条件
		/// </summary>
		public Lebi_ServicePanel_Type GetModel(string strWhere, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, "", "");
		   ...
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow, strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           model = ReaderBind(dataReader);
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_ServicePanel_Type",model.id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_ServicePanel_Type GetModel(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldShow = "*";
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_ServicePanel_Type> GetList(string strWhere, string strFieldOrder, int PageSize, int page, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as List<Lebi_ServicePanel_Type>;
		   }
		   ...
		   if (cachekey != "" && list.Count > 0)
		         LB.DataAccess.DB.Instance.SetMemchche(cachekey, list, "Lebi_ServicePanel_Type", 0 , cachestr , seconds);
		   return list;
		}
		public List<Lebi_ServicePanel_Type> GetList(SQLPara para, int PageSize, int page, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldKey = "id";
		   string strFieldShow = "*";
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_ServicePanel_Type> GetList(string strWhere,string strFieldOrder, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
		...
		public List<Lebi_ServicePanel_Type> GetList(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   StringBuilder strSql=new StringBuilder();
		...
		public Lebi_ServicePanel_Type BindForm(Lebi_ServicePanel_Type model)
		{
			...
			if (HttpContext.Current.Request["IsOnline"] != null)
				model.IsOnline=LB.Tools.RequestTool.RequestInt("IsOnline",0);
			...
			if (HttpContext.Current.Request["Text"] != null)
				model.Text=LB.Tools.RequestTool.RequestString("Text");
		...
		public Lebi_ServicePanel_Type SafeBindForm(Lebi_ServicePanel_Type model)
		{
			...
			if (HttpContext.Current.Request["Sort"] != null)
				model.Sort=LB.Tools.RequestTool.RequestInt("Sort",0);
			if (HttpContext.Current.Request["Text"] != null)
				model.Text=LB.Tools.RequestTool.RequestSafeString("Text");
			if (HttpContext.Current.Request["Url"] != null)
		...
		public Lebi_ServicePanel_Type ReaderBind(IDataReader dataReader)
		{
			...
			if(ojb != null && ojb != DBNull.Value)
			{
				model.IsOnline= Convert.ToInt32(ojb);
			}
