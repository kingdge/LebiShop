...
namespace DB.LebiShop
{
	...
	/// </summary>
	public partial class D_Lebi_User_Answer
	{
		static D_Lebi_User_Answer _Instance;
		public static D_Lebi_User_Answer Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            _Instance = new D_Lebi_User_Answer("Lebi_User_Answer");
		        }
		        return _Instance;
		    }
		    set
		...
		string TableName = "Lebi_User_Answer";
		public D_Lebi_User_Answer(string tablename)
		{
		    TableName = tablename;
		}
		...
		public string GetValue(string colName,string strWhere,int seconds=0)
		{
		   ...
		   try
		   {
		       ...
		       if (cachekey != "")
		           LB.DataAccess.DB.Instance.SetMemchche(cachekey, val, "Lebi_User_Answer", 0 , cachestr , seconds);
		   }
		   catch
		   {
		       val = "";
		...
		public int Add(Lebi_User_Answer model)
		{
			...
			strSql.Append(LB.DataAccess.DB.BaseUtilsInstance.ColName("Answer")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("User_id")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("User_Question_id")+")");
			strSql.Append(" values (");
			strSql.Append("@Answer,@User_id,@User_Question_id);select @@identity;");
		...
		public void Update(Lebi_User_Answer model)
		{
			...
			if((","+model.UpdateCols+",").IndexOf(",Answer,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("Answer")+"= @Answer");
			...
			if((","+model.UpdateCols+",").IndexOf(",User_Question_id,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("User_Question_id")+"= @User_Question_id");
		...
		/// 得到一个对象实体 by id
		/// </summary>
		public Lebi_User_Answer GetModel(int id, int seconds=0)
		{
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as Lebi_User_Answer;
		   }
		   Lebi_User_Answer model = null;
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow,  strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           ...
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_User_Answer",id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by where条件
		/// </summary>
		public Lebi_User_Answer GetModel(string strWhere, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, "", "");
		   ...
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow, strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           model = ReaderBind(dataReader);
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_User_Answer",model.id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_User_Answer GetModel(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldShow = "*";
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_User_Answer> GetList(string strWhere, string strFieldOrder, int PageSize, int page, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as List<Lebi_User_Answer>;
		   }
		   ...
		   if (cachekey != "" && list.Count > 0)
		         LB.DataAccess.DB.Instance.SetMemchche(cachekey, list, "Lebi_User_Answer", 0 , cachestr , seconds);
		   return list;
		}
		public List<Lebi_User_Answer> GetList(SQLPara para, int PageSize, int page, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldKey = "id";
		   string strFieldShow = "*";
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_User_Answer> GetList(string strWhere,string strFieldOrder, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
		...
		public List<Lebi_User_Answer> GetList(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   StringBuilder strSql=new StringBuilder();
		...
		public Lebi_User_Answer BindForm(Lebi_User_Answer model)
		{
			if (HttpContext.Current.Request["Answer"] != null)
				model.Answer=LB.Tools.RequestTool.RequestString("Answer");
			...
			if (HttpContext.Current.Request["User_Question_id"] != null)
				model.User_Question_id=LB.Tools.RequestTool.RequestInt("User_Question_id",0);
				return model;
		}
		/// <summary>
		/// 安全方式绑定对象表单
		...
		{
			if (HttpContext.Current.Request["Answer"] != null)
				model.Answer=LB.Tools.RequestTool.RequestSafeString("Answer");
			if (HttpContext.Current.Request["User_id"] != null)
				model.User_id=LB.Tools.RequestTool.RequestInt("User_id",0);
			if (HttpContext.Current.Request["User_Question_id"] != null)
				model.User_Question_id=LB.Tools.RequestTool.RequestInt("User_Question_id",0);
		...
		public Lebi_User_Answer ReaderBind(IDataReader dataReader)
		{
			...
			if(ojb != null && ojb != DBNull.Value)
			{
				model.User_Question_id= Convert.ToInt32(ojb);
			}
			return model;
		}
