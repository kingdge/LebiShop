...
{
	/// <summary>
	/// 数据访问类D_Lebi_Language_Tag_Version。
	/// </summary>
	public partial class D_Lebi_Language_Tag_Version
	{
		static D_Lebi_Language_Tag_Version _Instance;
		public static D_Lebi_Language_Tag_Version Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            _Instance = new D_Lebi_Language_Tag_Version("Lebi_Language_Tag_Version");
		        }
		        return _Instance;
		    }
		    set
		...
		string TableName = "Lebi_Language_Tag_Version";
		public D_Lebi_Language_Tag_Version(string tablename)
		{
		    TableName = tablename;
		}
		...
		public string GetValue(string colName,string strWhere,int seconds=0)
		{
		   ...
		   try
		   {
		       ...
		       if (cachekey != "")
		           LB.DataAccess.DB.Instance.SetMemchche(cachekey, val, "Lebi_Language_Tag_Version", 0 , cachestr , seconds);
		   }
		   catch
		   {
		       val = "";
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Lebi_Language_Tag_Version model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into "+ TableName + " (");
			strSql.Append(LB.DataAccess.DB.BaseUtilsInstance.ColName("Content")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Language_Code")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Time_Update")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Version")+")");
			strSql.Append(" values (");
			strSql.Append("@Content,@Language_Code,@Time_Update,@Version);select @@identity;");
		...
		/// 更新一条数据
		/// </summary>
		public void Update(Lebi_Language_Tag_Version model)
		{
			StringBuilder strSql=new StringBuilder();
			...
			if((","+model.UpdateCols+",").IndexOf(",Language_Code,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("Language_Code")+"= @Language_Code");
		...
		/// 得到一个对象实体 by id
		/// </summary>
		public Lebi_Language_Tag_Version GetModel(int id, int seconds=0)
		{
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as Lebi_Language_Tag_Version;
		   }
		   Lebi_Language_Tag_Version model = null;
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow,  strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           ...
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Language_Tag_Version",id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by where条件
		/// </summary>
		public Lebi_Language_Tag_Version GetModel(string strWhere, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, "", "");
		   ...
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow, strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           model = ReaderBind(dataReader);
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Language_Tag_Version",model.id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_Language_Tag_Version GetModel(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldShow = "*";
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_Language_Tag_Version> GetList(string strWhere, string strFieldOrder, int PageSize, int page, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as List<Lebi_Language_Tag_Version>;
		   }
		   List<Lebi_Language_Tag_Version> list = new List<Lebi_Language_Tag_Version>();
		   ...
		   if (cachekey != "" && list.Count > 0)
		         LB.DataAccess.DB.Instance.SetMemchche(cachekey, list, "Lebi_Language_Tag_Version", 0 , cachestr , seconds);
		   return list;
		}
		public List<Lebi_Language_Tag_Version> GetList(SQLPara para, int PageSize, int page, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldKey = "id";
		   string strFieldShow = "*";
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_Language_Tag_Version> GetList(string strWhere,string strFieldOrder, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
		...
		public List<Lebi_Language_Tag_Version> GetList(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   StringBuilder strSql=new StringBuilder();
		...
		/// 绑定对象表单
		/// </summary>
		public Lebi_Language_Tag_Version BindForm(Lebi_Language_Tag_Version model)
		{
			...
			if (HttpContext.Current.Request["Language_Code"] != null)
				model.Language_Code=LB.Tools.RequestTool.RequestString("Language_Code");
		...
		/// 安全方式绑定对象表单
		/// </summary>
		public Lebi_Language_Tag_Version SafeBindForm(Lebi_Language_Tag_Version model)
		{
			if (HttpContext.Current.Request["Content"] != null)
				model.Content=LB.Tools.RequestTool.RequestSafeString("Content");
			if (HttpContext.Current.Request["Language_Code"] != null)
				model.Language_Code=LB.Tools.RequestTool.RequestSafeString("Language_Code");
			if (HttpContext.Current.Request["Time_Update"] != null)
				model.Time_Update=LB.Tools.RequestTool.RequestTime("Time_Update", System.DateTime.Now);
			if (HttpContext.Current.Request["Version"] != null)
				model.Version=LB.Tools.RequestTool.RequestInt("Version",0);
		...
		/// 对象实体绑定数据
		/// </summary>
		public Lebi_Language_Tag_Version ReaderBind(IDataReader dataReader)
		{
			Lebi_Language_Tag_Version model=new Lebi_Language_Tag_Version();
			object ojb; 
			ojb = dataReader["id"];
