...
{
	/// <summary>
	/// 数据访问类D_Lebi_User_Card。
	/// </summary>
	public partial class D_Lebi_User_Card
	{
		static D_Lebi_User_Card _Instance;
		public static D_Lebi_User_Card Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            _Instance = new D_Lebi_User_Card("Lebi_User_Card");
		        }
		        return _Instance;
		    }
		    set
		...
		string TableName = "Lebi_User_Card";
		public D_Lebi_User_Card(string tablename)
		{
		    TableName = tablename;
		}
		...
		public string GetValue(string colName,string strWhere,int seconds=0)
		{
		   ...
		   try
		   {
		       ...
		       if (cachekey != "")
		           LB.DataAccess.DB.Instance.SetMemchche(cachekey, val, "Lebi_User_Card", 0 , cachestr , seconds);
		   }
		   catch
		   {
		       val = "";
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Lebi_User_Card model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into "+ TableName + " (");
			strSql.Append(LB.DataAccess.DB.BaseUtilsInstance.ColName("Card_id")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("User_id")+")");
			strSql.Append(" values (");
			strSql.Append("@Card_id,@User_id);select @@identity;");
			SqlParameter[] parameters = {
					new SqlParameter("@Card_id", model.Card_id),
		...
		/// 更新一条数据
		/// </summary>
		public void Update(Lebi_User_Card model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update "+ TableName + " set ");
			List<string> cols = new List<string>();
			if((","+model.UpdateCols+",").IndexOf(",Card_id,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("Card_id")+"= @Card_id");
		...
		/// 得到一个对象实体 by id
		/// </summary>
		public Lebi_User_Card GetModel(int id, int seconds=0)
		{
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as Lebi_User_Card;
		   }
		   Lebi_User_Card model = null;
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow,  strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           ...
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_User_Card",id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by where条件
		/// </summary>
		public Lebi_User_Card GetModel(string strWhere, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, "", "");
		   ...
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow, strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           model = ReaderBind(dataReader);
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_User_Card",model.id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_User_Card GetModel(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldShow = "*";
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_User_Card> GetList(string strWhere, string strFieldOrder, int PageSize, int page, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as List<Lebi_User_Card>;
		   }
		   List<Lebi_User_Card> list = new List<Lebi_User_Card>();
		   ...
		   if (cachekey != "" && list.Count > 0)
		         LB.DataAccess.DB.Instance.SetMemchche(cachekey, list, "Lebi_User_Card", 0 , cachestr , seconds);
		   return list;
		}
		public List<Lebi_User_Card> GetList(SQLPara para, int PageSize, int page, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldKey = "id";
		   string strFieldShow = "*";
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_User_Card> GetList(string strWhere,string strFieldOrder, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
		...
		public List<Lebi_User_Card> GetList(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   StringBuilder strSql=new StringBuilder();
		...
		/// 绑定对象表单
		/// </summary>
		public Lebi_User_Card BindForm(Lebi_User_Card model)
		{
			if (HttpContext.Current.Request["Card_id"] != null)
				model.Card_id=LB.Tools.RequestTool.RequestInt("Card_id",0);
		...
		/// 安全方式绑定对象表单
		/// </summary>
		public Lebi_User_Card SafeBindForm(Lebi_User_Card model)
		{
			if (HttpContext.Current.Request["Card_id"] != null)
				model.Card_id=LB.Tools.RequestTool.RequestInt("Card_id",0);
			if (HttpContext.Current.Request["User_id"] != null)
		...
		/// 对象实体绑定数据
		/// </summary>
		public Lebi_User_Card ReaderBind(IDataReader dataReader)
		{
			Lebi_User_Card model=new Lebi_User_Card();
			object ojb; 
			ojb = dataReader["id"];
			...
			ojb = dataReader["Card_id"];
			if(ojb != null && ojb != DBNull.Value)
			{
				model.Card_id= Convert.ToInt32(ojb);
			}
