...
namespace DB.LebiShop
{
	...
	/// </summary>
	public partial class D_Lebi_TabChild
	{
		static D_Lebi_TabChild _Instance;
		public static D_Lebi_TabChild Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            _Instance = new D_Lebi_TabChild("Lebi_TabChild");
		        }
		        return _Instance;
		    }
		    set
		...
		string TableName = "Lebi_TabChild";
		public D_Lebi_TabChild(string tablename)
		{
		    TableName = tablename;
		}
		...
		public string GetValue(string colName,string strWhere,int seconds=0)
		{
		   ...
		   try
		   {
		       ...
		       if (cachekey != "")
		           LB.DataAccess.DB.Instance.SetMemchche(cachekey, val, "Lebi_TabChild", 0 , cachestr , seconds);
		   }
		   catch
		   {
		       val = "";
		...
		public int Add(Lebi_TabChild model)
		{
			...
			strSql.Append(LB.DataAccess.DB.BaseUtilsInstance.ColName("num")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("protypeid")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("sort")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("tabid")+")");
			strSql.Append(" values (");
			strSql.Append("@num,@protypeid,@sort,@tabid);select @@identity;");
		...
		public void Update(Lebi_TabChild model)
		{
			...
			if((","+model.UpdateCols+",").IndexOf(",num,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("num")+"= @num");
			if((","+model.UpdateCols+",").IndexOf(",protypeid,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("protypeid")+"= @protypeid");
			if((","+model.UpdateCols+",").IndexOf(",sort,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("sort")+"= @sort");
			if((","+model.UpdateCols+",").IndexOf(",tabid,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("tabid")+"= @tabid");
		...
		/// 得到一个对象实体 by id
		/// </summary>
		public Lebi_TabChild GetModel(int id, int seconds=0)
		{
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as Lebi_TabChild;
		   }
		   Lebi_TabChild model = null;
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow,  strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           ...
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_TabChild",id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by where条件
		/// </summary>
		public Lebi_TabChild GetModel(string strWhere, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, "", "");
		   ...
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow, strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           model = ReaderBind(dataReader);
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_TabChild",model.id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_TabChild GetModel(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldShow = "*";
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_TabChild> GetList(string strWhere, string strFieldOrder, int PageSize, int page, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as List<Lebi_TabChild>;
		   }
		   ...
		   if (cachekey != "" && list.Count > 0)
		         LB.DataAccess.DB.Instance.SetMemchche(cachekey, list, "Lebi_TabChild", 0 , cachestr , seconds);
		   return list;
		}
		public List<Lebi_TabChild> GetList(SQLPara para, int PageSize, int page, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldKey = "id";
		   string strFieldShow = "*";
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_TabChild> GetList(string strWhere,string strFieldOrder, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
		...
		public List<Lebi_TabChild> GetList(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   StringBuilder strSql=new StringBuilder();
		...
		public Lebi_TabChild BindForm(Lebi_TabChild model)
		{
			if (HttpContext.Current.Request["num"] != null)
				model.num=LB.Tools.RequestTool.RequestInt("num",0);
			if (HttpContext.Current.Request["protypeid"] != null)
				model.protypeid=LB.Tools.RequestTool.RequestInt("protypeid",0);
			if (HttpContext.Current.Request["sort"] != null)
				model.sort=LB.Tools.RequestTool.RequestInt("sort",0);
			if (HttpContext.Current.Request["tabid"] != null)
				model.tabid=LB.Tools.RequestTool.RequestInt("tabid",0);
				return model;
		}
		/// <summary>
		/// 安全方式绑定对象表单
		...
		public Lebi_TabChild ReaderBind(IDataReader dataReader)
		{
			...
			if(ojb != null && ojb != DBNull.Value)
			{
				model.protypeid= Convert.ToInt32(ojb);
			}
			ojb = dataReader["sort"];
			if(ojb != null && ojb != DBNull.Value)
			{
			...
			if(ojb != null && ojb != DBNull.Value)
			{
				model.tabid= Convert.ToInt32(ojb);
			}
			return model;
		}
