...
namespace DB.LebiShop
{
	...
	/// </summary>
	public partial class D_Lebi_Supplier_Power
	{
		static D_Lebi_Supplier_Power _Instance;
		public static D_Lebi_Supplier_Power Instance
		{
		   get
		   {
		        if (_Instance == null)
		        {
		            _Instance = new D_Lebi_Supplier_Power("Lebi_Supplier_Power");
		        }
		        return _Instance;
		    }
		    set
		...
		string TableName = "Lebi_Supplier_Power";
		public D_Lebi_Supplier_Power(string tablename)
		{
		    TableName = tablename;
		}
		...
		public string GetValue(string colName,string strWhere,int seconds=0)
		{
		   ...
		   try
		   {
		       ...
		       if (cachekey != "")
		           LB.DataAccess.DB.Instance.SetMemchche(cachekey, val, "Lebi_Supplier_Power", 0 , cachestr , seconds);
		   }
		   catch
		   {
		       val = "";
		...
		public int Add(Lebi_Supplier_Power model)
		{
			...
			strSql.Append(LB.DataAccess.DB.BaseUtilsInstance.ColName("Supplier_Group_id")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Supplier_Limit_Code")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Supplier_Limit_id")+","+LB.DataAccess.DB.BaseUtilsInstance.ColName("Url")+")");
			strSql.Append(" values (");
			strSql.Append("@Supplier_Group_id,@Supplier_Limit_Code,@Supplier_Limit_id,@Url);select @@identity;");
		...
		public void Update(Lebi_Supplier_Power model)
		{
			...
			if((","+model.UpdateCols+",").IndexOf(",Supplier_Limit_Code,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("Supplier_Limit_Code")+"= @Supplier_Limit_Code");
			if((","+model.UpdateCols+",").IndexOf(",Supplier_Limit_id,")>-1 || model.UpdateCols=="")
			   cols.Add(LB.DataAccess.DB.BaseUtilsInstance.ColName("Supplier_Limit_id")+"= @Supplier_Limit_id");
		...
		/// 得到一个对象实体 by id
		/// </summary>
		public Lebi_Supplier_Power GetModel(int id, int seconds=0)
		{
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as Lebi_Supplier_Power;
		   }
		   Lebi_Supplier_Power model = null;
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow,  strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           ...
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Supplier_Power",id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by where条件
		/// </summary>
		public Lebi_Supplier_Power GetModel(string strWhere, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, "", "");
		   ...
		   using (IDataReader dataReader = LB.DataAccess.DB.Instance.TextExecuteReaderOne(strTableName, strFieldShow, strWhere, null))
		   {
		       ...
		       while (dataReader.Read())
		       {
		           model = ReaderBind(dataReader);
		           if (cachekey != "")
		               LB.DataAccess.DB.Instance.SetMemchche(cachekey, model, "Lebi_Supplier_Power",model.id , cachestr , seconds);
		           return model;
		       }
		   }
		   return null;
		...
		/// 得到一个对象实体 by SQLpara
		/// </summary>
		public Lebi_Supplier_Power GetModel(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldShow = "*";
		...
		/// 获得数据列表-带分页
		/// </summary>
		public List<Lebi_Supplier_Power> GetList(string strWhere, string strFieldOrder, int PageSize, int page, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		   ...
		   if (BaseUtils.BaseUtilsInstance.MemcacheInstance != null && seconds > 0)
		   {
		       ...
		       var obj = LB.DataAccess.DB.Instance.GetMemchche(cachekey);
		       if (obj != null)
		           return obj as List<Lebi_Supplier_Power>;
		   }
		   ...
		   if (cachekey != "" && list.Count > 0)
		         LB.DataAccess.DB.Instance.SetMemchche(cachekey, list, "Lebi_Supplier_Power", 0 , cachestr , seconds);
		   return list;
		}
		public List<Lebi_Supplier_Power> GetList(SQLPara para, int PageSize, int page, int seconds=0)
		{
		   string strTableName = TableName;
		   string strFieldKey = "id";
		   string strFieldShow = "*";
		...
		/// 获得数据列表-不带分页
		/// </summary>
		public List<Lebi_Supplier_Power> GetList(string strWhere,string strFieldOrder, int seconds=0)
		{
		   if (strWhere.IndexOf("lbsql{") > 0)
		   {
		       SQLPara para = new SQLPara(strWhere, strFieldOrder, "");
		...
		public List<Lebi_Supplier_Power> GetList(SQLPara para, int seconds=0)
		{
		   string strTableName = TableName;
		   StringBuilder strSql=new StringBuilder();
		...
		public Lebi_Supplier_Power BindForm(Lebi_Supplier_Power model)
		{
			...
			if (HttpContext.Current.Request["Supplier_Limit_Code"] != null)
				model.Supplier_Limit_Code=LB.Tools.RequestTool.RequestString("Supplier_Limit_Code");
			if (HttpContext.Current.Request["Supplier_Limit_id"] != null)
				model.Supplier_Limit_id=LB.Tools.RequestTool.RequestInt("Supplier_Limit_id",0);
		...
		public Lebi_Supplier_Power SafeBindForm(Lebi_Supplier_Power model)
		{
			if (HttpContext.Current.Request["Supplier_Group_id"] != null)
				model.Supplier_Group_id=LB.Tools.RequestTool.RequestInt("Supplier_Group_id",0);
			if (HttpContext.Current.Request["Supplier_Limit_Code"] != null)
				model.Supplier_Limit_Code=LB.Tools.RequestTool.RequestSafeString("Supplier_Limit_Code");
			if (HttpContext.Current.Request["Supplier_Limit_id"] != null)
				model.Supplier_Limit_id=LB.Tools.RequestTool.RequestInt("Supplier_Limit_id",0);
			if (HttpContext.Current.Request["Url"] != null)
		...
		public Lebi_Supplier_Power ReaderBind(IDataReader dataReader)
		{
			...
			if(ojb != null && ojb != DBNull.Value)
			{
				model.Supplier_Limit_id= Convert.ToInt32(ojb);
			}
